...
Scenario('Convert PDF, Validate JSON, and Open Browser', async () => {
  ...
  const pdfFilePath = 'InputExample3.pdf';
  ...
  I.wait(2);
  I.fillField(LOCATORS.searchbar.searchInput, "OC-26-1002-8403-00611617")
  I.wait(5);
  ...
  for (const group of groupedSections) {
    ...
    for (const section of group.items) {
      ...
      I.wait(8);
      const popup = await I.grabNumberOfVisibleElements(LOCATORS.popup);
      console.log("Grabbing popup",popup);
    if (popup > 0) {
      console.log("Popup visible");
      await I.click(LOCATORS.confirmYesBtn);
      await I.wait(4);
    }
...
//   // Logic to handle cases where multiple sections are in the DOM.
//   const numberOfMatchingElements = await I.grabNumberOfVisibleElements(dobInputLocator);
//   let finalDobLocator = dobInputLocator;

//   if (numberOfMatchingElements > 1) {
//     I.say(`⚠️ Ambiguous locator detected. Found ${numberOfMatchingElements} elements for the date input. Targeting the last one.`);
//     finalDobLocator = `(${dobInputLocator})[${numberOfMatchingElements}]`;
//   }

//   // ======== FIX STARTS HERE: ROBUST CLICKING LOGIC ========
//   I.say(`Opening date picker for locator: ${finalDobLocator}`);
//   await I.scrollTo(finalDobLocator); // First, ensure the element is scrolled into view.
//   await I.wait(0.5); // Add a brief pause for the UI to settle after scrolling.

//   try {
//     // Attempt the standard click.
//     await I.click(finalDobLocator);
//   } catch (err) {
//     // If the click is intercepted, this block will run.
//     if (err.message.includes('element click intercepted')) {
//       I.say('⚠️ Standard click was intercepted by another element. Retrying with a direct JavaScript click.');
//       // Fallback to a JS-based click, which is more resilient to overlapping elements.
...
//         const el = getElementByXPath(locator);
//         if (el) {
//           el.click();
//         }
//       }, finalDobLocator);
//     } else {
//       // If it's a different error, let the test fail as it's unexpected.
//       throw err;
//     }
//   }
//   // ======== FIX ENDS HERE ========
...
//   const monthName = monthNames[parseInt(extractedDateObj.month) - 1]; // Corrected: month is 1-based index
//   const monthSelector = `//div[contains(@class,"vdatetime-month-picker")]//*[normalize-space(text())="${monthName}"]`;
...
//     const continueBtn = '//div[contains(@class,"vdatetime-popup__actions__button--confirm")]';
//     await I.waitForElement(continueBtn, 5);
//     if(I.see('//div[contains(@class,"vdatetime-popup__actions__button--confirm")]'))
//       await I.click(continueBtn);
//     I.say('✅ Date (and time if present) confirmed.');
//     await I.wait(0.25);
//     I.wait(0.2);
...
//     if (selectedLower === "others") {
//         I.say("✔ 'Others' is already selected. Skipping.");
//         await I.click(dropdownXpath); // Close dropdown
//         return;
//     }
...
//   I.click(LOCATORS.searchbar.searchInput);
//   I.wait(2);
//   I.fillField(LOCATORS.searchbar.searchInput, "OC-26-1002-8402-00010259")
//   I.wait(5);
//   I.click(LOCATORS.searchbar.searchbtn);
//   I.say("Searching for case");
//   I.wait(5);
...
//           const doaVal = jsonData["DOA"] || '';
//           const dodVal = jsonData["DOD"] || '';
//           await fillTreatingDoctorVisitingDates(I, target, doaVal, dodVal);
...
//       if (typeof target === "object" && target.dropdown && target.option) {
//         I.say(`Selecting dropdown for ${key}`);
//         await I.scrollTo(target.dropdown);
//         await I.wait(0.2);
//         await I.click(target.dropdown);
//         await I.wait(0.5);
//         await I.click(target.option);
//         await I.wait(0.2);

//         if (target.remark) {
//           let remarkValue = "SELF"; // default
//           // A long list of keys that should have "VERIFIED" as remark
//           const verifiedKeys = new Set([
//               "Patient Id", "Patient Photos", "Medical Records", "Evidence", 
//               "Statement from Patient Audio / Video", "Statement from Patient - Document",
//               "Medical records from patient (Past & Present)", "Prescriptions past and current",
//               "Diet plan document", "Audio / Video recording - evidence", "IPD / OT Paper",
//               "OPD & IPD Register Photos", "Lab Report", "Room Type Photo", "Infrastructure Photos",
//               "Ambulance Record", "Pharmacy Records", "Prescriptions past and current1",
//               "Diet Plan Document", "Written Statement of Treating Doctor", "Medical Registration Copy",
//               "Consultation / Visiting hours - Document", "Doctor Attendence & Visit Register Photo",
//               "Audio / Video recording - Evidence", "Written Statement of Treating Doctor - Audio / Video",
//               "Written Statement of Treating Doctor - Document", "Backdated Records", "Lab Register Photo",
//               "Discharge Summary", "Statement 1 - Audio / Video / Doc", "Statement 2 - Audio / Video / Doc",
//               "Statement 3 - Audio / Video / Doc", "Audio / Video Recording - Evidence", "Hospital Bills",
//               "Cash Receipt", "Pharmacy Bills", "Ambulance Bills", "Lab / Radiology Bills",
//               "Radiology Register Photo", "Radiology Report"
//           ]);
//           if (verifiedKeys.has(key)) {
//             remarkValue = "VERIFIED";
//           }
//           I.say(`Filling remark for ${key} → ${remarkValue}`);
//           await I.fillField(target.remark, remarkValue);
//         }
//       }

//       // 🧩 2️⃣ Handle multiple input xpaths (arrays)
//       else if (Array.isArray(target) && valueFromJson !== undefined && valueFromJson !== null) {
//         let value = valueFromJson.toString().trim().replace(/\./g, "");
//         for (const xpath of target) {
//           await I.fillField(xpath, value);
//         }
//       }

//       // 🧩 3️⃣ Handle single input fields
//       else if (typeof target === "string" && valueFromJson !== undefined && valueFromJson !== null) {
//         let value = valueFromJson
//           .toString()
//           .trim()
//           .replace(/[./\\#@!$%^&*()_+=\[\]{}|:;"'<>,?~`]/g, "")
...
//             normalized = `${digits.substring(0, 2)}/${digits.substring(2, 4)}/${digits.substring(4)}`;
//           } else {
//             normalized = valueFromJson.toString().trim(); // Use original for better parsing
//           }
...
//   const groupedSections = [
//     {
//       mainOpen: formMapping.StatementfromPatient.openMain,
//       items: ["StatementfromPatient", "StatementPatientDocumentsPD", "HospitalDetails", "HospitalDocumentsPD", "StatementfromTreatingDoctor", "TreatingDocterDocumentsPD", "StatementfromFamilyDoctor", "FamilyDoctorDocumentsPD", "PathologyLabPathologistdetails", "PathologistDocumentsPD", "VicinityVerification", "VicinityVerificationDocumentsPD", "PaymentReceiptsverifications", "PaymentDocumentsPD", "RadiologyCenter", "RadiologyCenterDocumentsPD", "QualityCheck", "Trigger"],
//     }
//   ];
